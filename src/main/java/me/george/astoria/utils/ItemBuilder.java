...
public class ItemBuilder {
    ...
    public ItemBuilder(Item item) {
        ...
        if (item.getItem().hasItemMeta())
            this.meta = item.getItem().getItemMeta();
        this.material = item.getItem().getType();
        this.amount = item.getItem().getAmount();
        this.data = item.getItem().getData();
        this.damage = item.getItem().getDurability();
        this.enchantments = item.getItem().getEnchantments();
        if (item.getItem().hasItemMeta())
            this.displayname = item.getItem().getItemMeta().getDisplayName();
        if (item.getItem().hasItemMeta())
            this.lore = item.getItem().getItemMeta().getLore();
        if (item.getItem().hasItemMeta())
            for (ItemFlag f : item.getItem().getItemMeta().getItemFlags()) {
                flags.add(f);
            }
    }
